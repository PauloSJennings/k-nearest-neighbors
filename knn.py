...
def distancia_entre_dois_pontos(ponto1: tuple, ponto2: tuple):
    ...
    for i in range(len(ponto1)):
        termos.append((ponto2[i] - ponto1[i])**2)

    resultado = (sum(termos))**0.5

    return resultado


def classifica_investidor(investimentos: tuple, num_vizinhos: int):
    distancias = []
    vizinhos = []
    for i in range(len(data)):
        distancia = distancia_entre_dois_pontos(data[i][2], investimentos)
        distancias.append((distancia, i))

    distancias.sort()

    for item in distancias[:num_vizinhos]:
    ...
    for item in vizinhos:
        if(data[item][1] == 'Conservador'):
            conservador_cont += 1
        elif(data[item][1] == 'Moderado'):
            moderado_cont += 1
        else:
            agressivo_cont += 1


    if (conservador_cont > moderado_cont) and (conservador_cont > agressivo_cont):
        return 'Conservador'
    elif(moderado_cont > conservador_cont) and (moderado_cont > agressivo_cont):
        return 'Moderado'
    elif(agressivo_cont > conservador_cont) and (agressivo_cont > moderado_cont):
        return 'Agressivo'
    else:
        return classifica_investidor(investimentos, num_vizinhos - 1)   

for item in no_class:
    print(classifica_investidor(item[2], 5))